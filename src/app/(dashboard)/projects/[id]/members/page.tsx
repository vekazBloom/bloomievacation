...
export default async function ProjectMembersPage({ params }: { params: { id: string } }) {
  ...
  await reconcileAcceptedInvitationMemberships(service, params.id);
  ...
  for (const member of members || []) {
    ...
    if (email) {
      await closePendingInvitationsForEmail(service, params.id, email);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 animate-fade-in">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <AddExistingMemberForm projectId={params.id} />
      </div>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {(members || []).map((member: any) => (
            <MemberManagerRow key={member.id} projectId={params.id} member={member} />
          ))}
        </CardContent>
      </Card>
