-- ============================================================================
-- Restore per-project leave "used" counters on project_members.
-- Migration 008 moved sync_leave_balance() to user_leave_balances only; the
-- project overview table still reads project_members.*_leave_used — keep both
-- in sync (global row + per-membership row for the request's project).
-- ============================================================================
...
BEGIN
  ...
  IF TG_OP = 'INSERT' AND NEW.status = 'approved' THEN
    ...
    v_pm_project_id := NEW.project_id;
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.status = 'approved' AND NEW.status != 'approved' THEN
      ...
      v_pm_project_id := OLD.project_id;
...
-- Reconcile per-project used totals from approved requests (one-time fix for drift).
UPDATE public.project_members pm
SET
  annual_leave_used = COALESCE(
    (
      SELECT SUM(lr.working_days_count)
      FROM public.leave_requests lr
      WHERE lr.project_id = pm.project_id
        AND lr.user_id = pm.user_id
        AND lr.type = 'annual'
        AND lr.status = 'approved'
    ),
    0
  ),
  sick_leave_used = COALESCE(
    (
      SELECT SUM(lr.working_days_count)
      FROM public.leave_requests lr
      WHERE lr.project_id = pm.project_id
  ...
  religious_leave_used = COALESCE(
    (
      SELECT SUM(lr.working_days_count)
      FROM public.leave_requests lr
      WHERE lr.project_id = pm.project_id
        AND lr.user_id = pm.user_id
        AND lr.type = 'religious'
        AND lr.status = 'approved'
    ),
    0
  );
